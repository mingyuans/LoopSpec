...
模板正文中调用 `loopspec status` 的步骤 SHALL NOT 附加 `--json`：该命令的默认输出已是为 LLM 准备的 Markdown 报告，让 skill 继续要求 `--json` 会使 Agent 绕开该报告去解析 JSON。正文中调用其他子命令（如 `loopspec instructions`）的步骤 SHALL 继续附加 `--json`。
