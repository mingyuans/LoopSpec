...
任何命令执行失败 SHALL 以退出码 `1` 结束，并在结构化模式下输出包含 `error`（机器可读错误码）、`message`（人类可读说明）、`fix`（可直接执行的修复建议）三个字段的 JSON。不带 `--json` 时，失败输出 SHALL 以 `=== ERROR ===` 分隔行开头，并逐项给出同样的错误码、说明与修复建议三项内容——分隔行的形式与 `status` 报告的分节标记一致（见 `status-report` 能力），使两类输出在版式上同构。此约定对**全部**子命令生效，而不仅是 `status`。
...
- **THEN** 命令以退出码 1 结束，stdout 输出以 `=== ERROR ===` 分隔行开头，并含 `error`、`message`、`fix` 三项内容
