...
`WorkflowConfig` 是 `extra: "forbid"`，因此这必须是显式声明的字段。未知节名（如拼成 `nodess`）SHALL 告警而不中断——与 `loopspec-cli` 现有「`rules` 引用不存在的节点 ID 时输出告警但不中断」的处理一致，复用同一套告警通道。
...
3. `WorkflowConfig` 新增 `status_report_prompts` 字段与未知节名告警（D13），渲染器按 D14 逐行消毒并缩进后追加输出。
