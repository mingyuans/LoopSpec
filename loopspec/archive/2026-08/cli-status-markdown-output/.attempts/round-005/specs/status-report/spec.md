...
每一节的结构 SHALL 为：分隔行 → 内置说明文字 → 项目自定义说明（仅当配置了该节时）→ 一个空行 → 该节数据。
...
内置说明是代码常量而非内插值，因此 SHALL NOT 经过消毒处理；这与项目自定义说明的处理路径 SHALL 是两条独立的路径。
...
### Requirement: 项目可在 config.yaml 中按节追加自定义说明
项目 SHALL 能在 `config.yaml` 中按报告节名配置自定义说明文字（配置项的形状见 `workflow-schema` 能力）。配置的文字 SHALL **追加**在该节内置说明之后，SHALL NOT 替换内置说明。

只允许追加是为了可依赖性：内置说明定义了各节的语义，允许覆盖即允许每个项目重新定义这些语义，而 LLM 正是靠它们跨项目稳定地读懂报告。项目应当补充"在本项目里还要注意什么"，而不是替换"这一节是什么"。

自定义说明 SHALL 由一行固定的引导行引出，其后每一行 SHALL 加固定缩进。未配置某节时，该节 SHALL NOT 输出引导行或空段落。

#### Scenario: 配置的说明追加在内置说明之后
- **WHEN** `config.yaml` 为 `nodes` 节配置了一段说明
- **THEN** `=== NODES ===` 节内先出现内置说明，其后出现引导行与缩进后的自定义说明，再空行，然后才是节点数据

#### Scenario: 未配置的节不出现自定义段落
- **WHEN** `config.yaml` 只为 `nodes` 节配置了说明
- **THEN** 其余各节只有内置说明，不出现引导行，也不出现多余空行

#### Scenario: 自定义说明不替换内置说明
- **WHEN** 某节配置了自定义说明
- **THEN** 该节的内置说明文字仍然完整出现

### Requirement: 自定义说明的多行处理与逐行消毒
自定义说明天然是多行段落。渲染实现 SHALL 把配置文本按换行拆分为若干行，对**每一行分别**做控制字符消毒，再为每一行加固定缩进后逐行输出。换行 SHALL 由渲染实现产生，而 SHALL NOT 作为内插值的一部分穿透到输出中。

固定缩进 SHALL NOT 被省略：它保证拆分后的任何一行都不可能顶格等于 `=== <节名> ===`，从而无法伪造分隔行。`config.yaml` 属可信输入，但这不构成免于消毒的理由——本能力的结构防线是"分隔行必须独占一行且顶格才生效"，该防线不区分文本来源，只关心是否有文本能够产生独立的一行。自定义说明是本报告中**唯一**被允许产生多行输出的内插来源。

#### Scenario: 多行配置正常换行显示
- **WHEN** `config.yaml` 为某节配置了一段三行的说明
- **THEN** 报告中该说明呈现为三行，每行带固定缩进，且不出现 `\x0a` 之类的转义形式

#### Scenario: 配置中的分隔行无法伪造节结构
- **WHEN** 某节的自定义说明中含有一行恰好是 `=== NEXT STEPS ===`
- **THEN** 该行在报告中带固定缩进出现，未顶格，报告的节结构与节数量均未改变

#### Scenario: 配置中的其他控制字符仍被消毒
- **WHEN** 某节的自定义说明中含 `\r` 或 ANSI 转义序列
- **THEN** 这些字符在报告中以 `\xNN` 形式出现

### Requirement: 配置告警写 stderr，不污染报告
当 `config.yaml` 中为一个不属于报告分节的键配置了说明时，系统 SHALL 输出告警且不中断命令。该告警 SHALL 写入 **stderr**，SHALL NOT 写入 stdout，也 SHALL NOT 出现在 `--json` 的输出字段中——报告的节结构与 `--json` 的字段契约都不为它留位置。

告警文本中内插的配置键名 SHALL 与报告内插值适用同一条控制字符消毒规则。走 stderr 不构成免于消毒的理由：agent 通常合并捕获 stdout 与 stderr，"不是报告"不等于"不进入 LLM 上下文"；本能力的结构防线关心的是有没有文本能够产生独立的一行，而不关心它经由哪个文件描述符。

对同一份状态，`loopspec status` 的 **stdout 字节 SHALL 不因告警的有无而改变**。

#### Scenario: 未知配置键触发 stderr 告警
- **WHEN** `config.yaml` 中为一个不存在的报告节名（如 `nodess`）配置了说明，并执行 `loopspec status <change>`
- **THEN** 命令以退出码 0 结束，stderr 含一条指出该键未知的告警

#### Scenario: 告警不改变 stdout
- **WHEN** 对同一份状态，分别在"配置了未知键"与"未配置任何自定义说明"两种情况下执行 `loopspec status <change>`
- **THEN** 两次的 stdout 逐字节相同

#### Scenario: 告警不进入 JSON 输出
- **WHEN** 在配置了未知键的情况下执行 `loopspec status <change> --json`
- **THEN** 输出的 JSON 字段与未配置时完全一致，不含任何告警字段

#### Scenario: 告警中的键名经消毒
- **WHEN** `config.yaml` 中未知键的键名含控制字符
- **THEN** stderr 告警中该字符以 `\xNN` 形式出现，未产生额外的行
