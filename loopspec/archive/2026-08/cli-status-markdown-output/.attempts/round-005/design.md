...
每节的结构为：分隔行 → 内置说明文字（见 D12）→ 项目自定义说明（若配置了，见 D13）→ 空行 → 该节数据。
...
### D13 · `config.yaml` 按节追加，不覆盖

`WorkflowConfig` 新增 `status_report_prompts: dict[str, str]`（YAML 键 `status_report_prompts`），按**报告节名的小写连字符形式**索引：`overview`、`nodes`、`gate-failures`、`pending-rollback`、`next-steps`、`error`。

```yaml
status_report_prompts:
  nodes: |
    This project keeps its API contracts under specs/api/.
    A specs node is not done until every endpoint has a contract.
  next-steps: |
    Always run commands from the repository root.
```

**只追加，不覆盖。** 内置说明先输出，项目自定义紧随其后。理由是可依赖性：内置说明定义了报告各节的**语义**（"NODES 节列出全部节点、列序是 id/status/output/notes"），允许覆盖就等于允许每个项目重新定义这些语义，而 LLM 恰恰要靠它们跨项目稳定地读懂报告。项目该说的是"在本项目里还要注意什么"，那是补充而非替换。若某项目确实认为内置说明有误，正确的出口是改 loopspec 本身，不是在自己的 config 里悄悄改写。

`WorkflowConfig` 是 `extra: "forbid"`，因此这必须是显式声明的字段。未知节名（如拼成 `nodess`）SHALL 告警而不中断——语义上与 `loopspec-cli` 现有「`rules` 引用不存在的节点 ID 时输出告警但不中断」一致，但**告警的出口不同**，见 D15。

**为什么放在 `config.yaml` 而不是 schema**：说明文字讲的是"本项目的约定"，与选用哪个 schema 无关；同一个 schema 在不同项目里应当能配不同的补充说明。这也是 `context`/`rules` 已经确立的分工。

### D14 · 用户说明的多行处理：逐行消毒 + 逐行缩进

用户在 YAML 里写的说明天然是多行段落，而 D10 的消毒会把换行改写成 `\x0a`——直接内插会把一段话压成一行乱码。

解法与 D10 已写下的思路同源，不是新发明：**换行由渲染器解析，而不是被内插**。渲染器把用户文本按 `\n` 拆成若干行，对**每一行**分别消毒（该行内残留的 `\r`、`\x1b` 等仍被改写为 `\xNN`），再给每一行加固定两格缩进后输出。

- 多行可读性保住了：换行是渲染器主动产生的，不是从内插值里"漏"出来的。
- 防线保住了：每一行都带两格缩进，因此任何一行都不可能等于 `=== ... ===`——分隔行必须独占一行、顶格才生效。

**`config.yaml` 可信不等于免消毒。** 这一点必须写死在 spec 里而不是靠实现者自觉：`security` 第 3 轮确认的防线是"分隔行必须独占一行才生效"，那条防线不关心文本来源可不可信，只关心有没有东西能造出独立的一行。用户说明恰恰是本设计里**唯一**一处被允许产生多行输出的内插值，因此它是这条防线上最需要盯住的位置。

内置说明不经消毒——它是代码里的常量，不是内插值。这个区别要在实现里体现为两条不同的路径，而不是"反正都过一遍"。

### D15 · 未知配置键的告警走 stderr（回应 `security` 第 4 轮阻塞）

D13 原本写的是"复用 `rules` 引用未知节点 ID 的既有告警通道"，但那条通道在 `status` 上并不存在：`rules` 的告警在 `instructions.py` 里构造，只进 `loopspec instructions` 响应的 `warnings` 数组，而 `cli.status` 的 payload 没有 `warnings` 字段。三条可能的出口各自撞线：

| 出口 | 撞上什么 |
|---|---|
| `status` payload 加 `warnings` | 违反冻结决策"`--json` 字段契约一字不改" |
| 印进报告 stdout | 要内插用户写的配置键名，而消毒需求列举的取值里没有它；且报告节结构无处容纳该行 |
| stderr | 规格通篇只约束 stdout，stderr 是否消毒、是否影响字节稳定性未定义 |

**选 stderr，并把未定义的部分补齐。** 它是三者中唯一不推翻既有决策的：不碰 `--json` 契约，不改报告的节结构，也不影响"两次渲染 stdout 逐字节相同"那些断言——因为它们只看 stdout。

补齐三点，缺一不可：

1. 告警写 **stderr**，不写 stdout。
2. 告警文本中内插的配置键名**同样经消毒**。理由不能含糊：stderr 一样会被 agent 捕获并进入 LLM 上下文，"不是报告"不等于"不进模型"。这条防线关心的是有没有文本能造出独立的一行，不关心它走哪个文件描述符。
3. **stdout 的字节不因告警的有无而改变**——同一份状态，配了未知键与没配，`loopspec status` 的 stdout 必须逐字节相同。这条要写成可测 scenario，否则"走 stderr"很容易在实现时退化成"两边都印一份"。

`instructions` 那边 `rules` 的既有告警行为不变——它有 `warnings` 字段可用，本次不动它。两条命令的告警出口不同，是因为它们的响应结构本来就不同，不是不一致。
...
- **[用户自定义说明是唯一被允许产生多行输出的内插值]** → 它因此是"分隔行必须独占一行"这条防线上最需要盯住的位置。缓解见 D14：逐行消毒 + 逐行两格缩进，缩进使任何一行都不可能顶格等于 `=== ... ===`。针对性测试见任务 5.15。
- **[stderr 的告警同样进入 LLM 上下文]** → agent 通常合并捕获 stdout 与 stderr，因此"走 stderr"不等于"不进模型"。缓解见 D15：告警文本中的配置键名同样经消毒，且 stdout 字节不受告警有无影响（后者写成可测 scenario，防止实现退化成两边各印一份）。
- **[内置说明文字随实现漂移]** → 说明文本决定 LLM 如何解读整份报告，散落在实现里就会被随手改。缓解：字面文本定在本文档的 Rendered Examples 中，spec 只要求"存在且覆盖全部节"，任务 2.9 要求实现逐字对照。
...
1. 实现 `status_report.py`（含 D10 的统一消毒入口、D12 的内置说明常量）与 `cli.status` 的分支切换，`--json` 路径不动。
2. 改 `cli._fail` 的非 JSON 分支，走同一条消毒规则（D7、D10）。
3. `WorkflowConfig` 新增 `status_report_prompts` 字段（D13），未知节名告警写 stderr 且键名经消毒（D15），渲染器按 D14 逐行消毒并缩进后追加输出。
4. 同步 `policy.py`/`cli.py` 中指向 `status` 的 `nextSteps` 文案（D8）。
5. 同步 `skill_templates.py`（D9）。
6. 同步 `README.md`、`docs/en/**`、`docs/zh/**` 中的 `loopspec status` 示例、CLI 参考与配置参考。

`status_report_prompts` 是可选字段且默认为空，因此既有的 `config.yaml` 无需改动即可继续工作——没有配置迁移步骤。

回滚策略：改动集中在一个新模块、一个可选配置字段与若干处文案，`git revert` 即可完全回退；`--json` 未被触碰，因此回退不影响任何已按 JSON 契约集成的调用方。

## Open Questions

- **glob 节点的详略权衡**（D4）：`existingOutputPaths` 压成计数是否可接受，还是 glob 节点应完整列出匹配文件。设计选压缩；`approval` 第 1、2 轮均未表态，留待第 3 轮确认。

（分节记法已由 `approval` 第 1 轮裁定见 D3；用户说明"追加 vs 覆盖"已由本轮 D13 裁定为只追加；二者不再是开放问题。）

## Rendered Examples

以下是渲染器输出的**字面文本**，供人核对版式。样例覆盖全部五个节、全部四种备注取值、以及每一节的内置说明文字——那些说明的措辞是设计的一部分（见 D12），实现须逐字照搬。

### 样例一：正常态（无 gate 失败，无项目自定义说明）
...
two; get the real paths from `loopspec instructions`.
...
### 样例二：`security` gate 失败态（含项目自定义说明）

此样例的 `config.yaml` 配置了：
...
项目自定义说明由 `Project notes:` 一行引出，随后每行两格缩进——缩进正是 D14 里那条防线：任何一行都不可能顶格等于 `=== ... ===`。该段落只在对应节配了内容时出现。
...
以上样例中的说明即为全部内置文本。按节名（也是 `config.yaml` 中的键名）汇总，供 spec 与实现引用：

| 节 | `config.yaml` 键 | 说明要点 |
|---|---|---|
| `=== OVERVIEW ===` | `overview` | change 位置与是否完成；本节路径是绝对路径，其余节是相对路径 |
| `=== NODES ===` | `nodes` | 列序含义、五种状态各自的意思、"别自己挑节点、照 NEXT STEPS 做"、`{a,b}` 是 gate 的二选一显示形式 |
| `=== GATE FAILURES ===` | `gate-failures` | 有 gate 驳回了工作；每块给出原因与回退会重置哪些节点；回退会归档这些节点的现有产物 |
| `=== PENDING ROLLBACK ===` | `pending-rollback` | 回退可用且是唯一出路；原样执行该命令后重新读状态 |
| `=== NEXT STEPS ===` | `next-steps` | 按序执行，第一条足以推进；照抄而不要自行组合命令 |
| `=== ERROR ===` | `error` | 命令已失败且未做任何改动；`error` 是稳定机器码，`fix` 是建议命令；先修因再重试 |
