...
- **glob 节点的详略权衡**（D4）：`existingOutputPaths` 压成计数是否可接受，还是 glob 节点应完整列出匹配文件。设计选压缩；`approval` 前三轮均未表态，留待下一轮确认。

（分节记法已由 `approval` 第 1 轮裁定，见 D3；项目级自定义分节说明已由 `approval` 第 3 轮裁定为本次不做，原 D13–D15 已删除；二者不再是开放问题。）
...
| `=== NODES ===` | 列序含义、五种状态各自的意思、"别自己挑节点、照 NEXT STEPS 做"、`{a,b}` 是 gate 的二选一显示形式 |
