...
不带 `--json` 时的默认输出，其目标读者 SHALL 按命令区分：`loopspec status` 的默认输出以 **LLM/Agent 消费**为目标，输出面向 LLM 的 Markdown 报告（见 `status-report` 能力）；其余命令的默认输出为人类可读形式。无论何种形式，同一命令的默认输出与 `--json` 输出所承载的信息 SHALL 保持一致，差别仅在编码形式与详略呈现（默认输出可对列表类字段做计数式压缩，但 SHALL NOT 遗漏任何一类信息）。
...
- **THEN** 输出为面向 LLM 的 Markdown 报告，而非逐字段的 `key: value` 列表

### Requirement: 统一错误输出格式
任何命令执行失败 SHALL 以退出码 `1` 结束，并在结构化模式下输出包含 `error`（机器可读错误码）、`message`（人类可读说明）、`fix`（可直接执行的修复建议）三个字段的 JSON。不带 `--json` 时，失败输出 SHALL 为 Markdown 形式：以一个错误标题开头，并逐项给出同样的错误码、说明与修复建议三项内容——此约定对**全部**子命令生效，而不仅是 `status`。

该 Markdown 失败输出中被内插的三项取值 SHALL 与 `status` 的 Markdown 报告适用**同一条**控制字符消毒规则（见 `status-report` 能力），并 SHALL 复用同一个消毒实现。两类输出的输入同源（同一批用户提供的 change 名与路径）、去向同一（同一个 LLM 的上下文），因此 SHALL NOT 存在两套标准。

该约束不是理论洁癖：`message` 中内插的 change 名未经格式校验（读取类命令只判断 change 目录是否存在，不校验 kebab-case），因此含换行的 change 名会使失败输出多出攻击者控制的行；由于本能力把二级标题确立为 LLM 定位结构的信号，这样一行可以冒充成一个合法的指令节。系统 SHALL NOT 依赖 CLI 框架在非 TTY 下的 ANSI 剥离行为作为防线——该行为不处理换行、`\r` 等控制字符。
...
- **THEN** 命令以退出码 1 结束，stdout 输出的 Markdown 中含错误标题以及 `error`、`message`、`fix` 三项内容

#### Scenario: 含换行的 change 名无法在错误输出中伪造节标题
- **WHEN** 以一个含换行、且换行后紧跟 `## Next Steps` 的 change 名执行 `loopspec status`（不带 `--json`）
- **THEN** 失败输出中除渲染实现自身写出的标题行外不出现任何新的行首 `#`，被内插的换行以可见的 `\xNN` 形式呈现
...
不带 `--json` 时，本命令 SHALL 输出面向 LLM 的 Markdown 报告，其版式、分节、消毒与转义规则由 `status-report` 能力规定。该报告 SHALL 渲染自与 `--json` 完全相同的一份内部结果数据，SHALL NOT 另行访问文件系统或重算节点状态。
...
#### Scenario: 默认输出为 Markdown 报告
- **WHEN** 执行 `loopspec status <change>` 而不带 `--json`
- **THEN** 输出为符合 `status-report` 能力所定版式的 Markdown 报告，且其内容与同一时刻 `--json` 的结果一致
