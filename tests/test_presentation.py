...
def welcome_output(**console_kwargs) -> str:
    presenter, stream = make_presenter(**console_kwargs)
    render_welcome(presenter)
    return stream.getvalue()


def test_welcome_section_order():
    output = welcome_output()
    positions = [
        output.index(LOGO_UNICODE[0]),
        output.index(WELCOME_TITLE),
        output.index(WELCOME_SUBTITLE),
        output.index("This setup will configure:"),
        output.index("Quick start after setup:"),
        output.index(WELCOME_PROMPT),
    ]
    assert positions == sorted(positions), output


def test_welcome_lists_what_setup_configures():
    output = welcome_output()
    for item in WELCOME_CONFIGURES:
        assert item in output


def test_welcome_quick_start_lists_each_command_with_a_purpose():
    output = welcome_output()
    for verb, purpose in WELCOME_QUICK_START:
        assert f"/lpsx:{verb}" in output
        assert purpose in output


def test_welcome_quick_start_only_names_commands_init_actually_writes():
    """A quick start that lists a command the user cannot type is worse than
    none. This is the assertion that keeps the two in step."""

    generated = {template.verb for template in SKILL_TEMPLATES}
    advertised = {verb for verb, _ in WELCOME_QUICK_START}
    assert advertised <= generated, f"not generated by init: {advertised - generated}"


def test_welcome_ends_by_pointing_at_the_picker():
    assert welcome_output().rstrip().endswith(WELCOME_PROMPT)
...
def test_logo_is_rendered_once_with_no_cursor_control():
    output = welcome_output()
    assert output.count(LOGO_UNICODE[0]) == 1
    assert "\x1b[" not in output  # no colour, and nothing that could redraw
    assert "\r" not in output


def test_logo_falls_back_to_ascii_when_the_encoding_cannot_take_it():
    stream = io.StringIO()
    presenter = Presenter(
        Console(file=stream, no_color=True, force_terminal=False, width=200), ascii_only=True
    )
    ...
    assert LOGO_ASCII[0] in output
    assert LOGO_UNICODE[0] not in output
    assert all(ord(char) < 128 for char in output), "ASCII mode must stay ASCII"
...
def test_logo_has_the_same_row_count_in_both_alphabets():
    assert len(LOGO_UNICODE) == len(LOGO_ASCII)


@pytest.mark.parametrize("logo", [LOGO_UNICODE, LOGO_ASCII])
def test_logo_rows_are_equal_width(logo: tuple[str, ...]):
    """Ragged rows read as a rendering bug; hand-editing the art is how that
    happens, so the widths are pinned."""

    assert len({len(row) for row in logo}) == 1, [len(row) for row in logo]


def test_presentation_exposes_no_animation_interface():
    """An animated logo was ruled out; the module must offer nothing that could
    drive one, so a later change cannot reach for it casually."""

    import loopspec.presentation as presentation

    banned = ("animate", "frames", "spin_logo", "redraw", "clear", "live", "loop", "sleep")
    exported = [name for name in dir(presentation) if not name.startswith("_")]
