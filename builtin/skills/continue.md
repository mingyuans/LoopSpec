...
4. Re-run `loopspec status <change-name>` and repeat from step 2 until `isComplete` is `true` or a gate is `exhausted`.

If a gate is `failed`, `nextSteps` will point you at `loopspec rollback <change-name> --json` first -- run it, then continue via `/lpsx:continue` (or the `loopspec-continue` skill); `loopspec instructions` for the reset nodes will include `priorAttempts` explaining what failed last time.
