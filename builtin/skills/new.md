...
3. Run `loopspec status <changeName>` using the returned canonical name to see the first ready node and its `nextSteps`.
4. Continue with `/lpsx:continue` -- or, if your tool has no slash commands, by invoking the `loopspec-continue` skill -- to drive the rest of the loop.
