...
3. Run that command. If it's `instructions`, write the artifact it describes to `resolvedOutputPath` (or `resolvedOutputPath.pass`/`.fail` for a gate), then update `state.md` per the instructions.
