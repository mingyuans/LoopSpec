...
`loopspec init [path]` SHALL 在指定路径（缺省为 `./loopspec`）创建 workflow home，包含 `config.yaml`（含默认 `artifacts_dir: changes` 与默认 `schema`）、空的 `schemas/` 目录与空的 `changes/` 目录；默认 SHALL 同时把内置 schema 复制到 `schemas/` 下；传入 `--no-builtin` SHALL 跳过复制，只生成空骨架。
